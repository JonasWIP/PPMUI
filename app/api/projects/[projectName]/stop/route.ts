...
// POST handler for stopping a project
export async function POST(
  ...
  try {
    ...
    const result = await ProjectsService.postProjectsStop({ projectName });
    
    return NextResponse.json(result);
  } catch (error) {
    console.error('Error in projects stop API route:', error);
    return NextResponse.json(
