...
// POST handler for cloning a repository
export async function POST(request: Request) {
  try {
    ...
    const result = await ProjectsService.postProjectsClone({ requestBody });
    
    return NextResponse.json(result);
  } catch (error) {
    console.error('Error in projects clone API route:', error);
    return NextResponse.json(
